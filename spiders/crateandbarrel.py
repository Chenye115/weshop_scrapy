...
website = 'crateandbarrel'
# 全流程解析脚本

class CrateandbarrelSpider(scrapy.Spider):
    name = website
    allowed_domains = ['crateandbarrel.net']
    start_urls = ['https://www.crateandbarrel.com/']
    ...
    def __init__(self, **kwargs):
        super(CrateandbarrelSpider, self).__init__(**kwargs)
        self.counts = 0
        setattr(self, 'author', "")
    ...
    def parse_init(self,response):
        #该网站的特殊性
        url_list=response.xpath("//li[@class='primary-nav-item']/div[1]/a/@href").getall()
    ...
    def parse(self, response):
        #找各个页面url
        url_list = response.xpath("//ul[@class='super-category-nav-main']//a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        print(url_list)
        for url in url_list:
            print(url)
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//ul[@class='card-deck-container break-out']//a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        print(url_list)
        for url in url_list:
            print(url)
    ...
    def parse_detail(self, response):
        ...
        original_price_path = response.xpath("//div[@class='hidden-xs']/div/div[@class='shop-bar-price-area']/span/span/span").getall()
        price_nums=len(original_price_path)
        if price_nums==1:
            original_price=original_price_path.xpath("./text()").get()
            original_price=original_price.split("$")[1]
            current_price=''
        else:
            original_price= original_price_path.xpath[1]("./text()").get()
            current_price = original_price_path.xpath[2]("./text()").get()
        ...
        items["name"] = response.xpath("//div[@class='hidden-xs']/div/h1[@class='product-name']/text()").get()
        attributes = list()
