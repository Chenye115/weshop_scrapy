...
class soniccouture(scrapy.Spider):
    name = website
    #allowed_domains = ['soniccouture.com']
    #start_urls = ['https://www.soniccouture.com/en/']
    ...
    def __init__(self, **kwargs):
        super(soniccouture, self).__init__(**kwargs)
        self.counts = 0
    ...
    def parse(self, response):
        url_list_label= response.xpath("//div[@class='product_menu_inner']/div")
        for url_label in url_list_label:
            cata=url_label.xpath("./div[1]/text()").get()
            print(cata)
            url_list = url_label.xpath("./div[2]//a/@href").getall()
            print(url_list)
            for url in url_list:
                url = 'https://www.soniccouture.com'+url
                print(url)
                yield scrapy.Request(
                    url=url,
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath("//label[@id='FM_price']/span[1]/span/text()").get()
        if original_price:
            original_price=original_price.strip().replace("$","")

        current_price = response.xpath("//label[@id='FM_price']/span[1]/text()").get()
        if current_price:
       ...
       # original_price=current_price
        ...
        items["brand"] = 'Soniccouture'
        name = response.xpath('//div[@id="lessmain"]/div[1]//text()').getall()
        name=self.filter_text(self.filter_html_label(''.join(name)))
        items["name"]=name
        # attributes = response.xpath('//div[@id="lessmain"]/div[2]//li/text()').getall()
        # if attributes:
        #     items["attributes"] = attributes
        # else:
        #     attributes=response.xpath("//div[@id='lessmain']/div[1]//div[@class='content']//li/text()").getall()
        #     print('next attr')
        #     items["attributes"] = attributes
        # items["about"] = response.xpath("").get()
        des=response.xpath('//div[@class="be_table_wrap"]//text()').getall()
        ...
        items["source"] = 'soniccouture.com'
        images_list = response.xpath("//div[@id='innerwrapper']//img/@src").getall()
        images=[]
        for n in range(len(images_list)):
            # images_label=re.search(r"((.*?))",images_list[n]).group(1)
            # print(images_label)
            # images.append('https://www.soniccouture.com'+images_label)
            images_list[n]='https://www.soniccouture.com'+images_list[n].replace("mobileimg","img")
