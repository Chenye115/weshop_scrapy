...
class BestfitsuSpider(scrapy.Spider):
    name = website
    allowed_domains = ['bestfitsu.com']
    start_urls = ['https://bestfitsu.com/']
    ...
    def __init__(self, **kwargs):
        super(BestfitsuSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//div[@class='products_cont js-list_product']/div/a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
        ...
        next_page_url = response.xpath("//div[@class='pager js-pagination pagination']/ul/li[@class='active']/following-sibling::li[1]/a/@href").get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        attributes = response.xpath("//div[@class='wrap-content']/ul/li//text()").getall()
        print(attributes)
        for n in range(len(attributes)):
            attributes[n]=self.filter_text(self.filter_html_label(attributes[n]))
        items["attributes"] = attributes

        # items["about"] = response.xpath("").get()
        # items["description"] = response.xpath("").get()

        items["source"] = 'bestfitsu.com'
        images_list = response.xpath("//*[@id='form_singleProduct']/div[1]//img/@src|//*[@id='form_singleProduct']/div[1]//img/@data-lazy").getall()#data-lazy
        images_list=list(set(images_list))
        items["images"] = images_list

        Breadcrumb_list = response.xpath("//div[@class='pr-breadcrumbs']//a/text()").getall()
        ...
        sku_option=response.xpath("//div[@class='js-product-sku product-sku js-empty-sku-view ']/div/div[2]")

        win_dic=re.search(r'window.skuAttr = \{(.*?)};',response.text).group(1)
        print(win_dic)

        win_dic=json.loads("{"+win_dic+"}")
        print(win_dic)
        ...
        if len(have_sku)>0:
            ...
            for i in range(len(have_sku)):
                sku_option_list = sku_option[i].xpath(".//span/@data-title").getall()
                if sku_option_list:
