...
class TumbltrakSpider(scrapy.Spider):
    name = website
    allowed_domains = ['tumbltrak.com']
    start_urls = ['https://tumbltrak.com/']
    ...
    def __init__(self, **kwargs):
        super(TumbltrakSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        ...
        #url = "http://tumbltrak.com/"
        #yield scrapy.Request(
        #    url=url,
        #)

    def parse(self, response):
        url_list = response.xpath("//ul[@class='navPages-list']//a/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//li[@class='product']/article/figure/a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath("//div[@class='productView-price productView-price--desktop ']//span[@class='price price--non-sale']/text()").get()
        ...
        current_price = response.xpath("//div[@class='productView-price productView-price--desktop ']//span[@class='price price--withoutTax']/text()").get()
        ...
        items["brand"] = 'tumbltrak'
        items["name"] = response.xpath("//div[@class='productView-product']/h1/text()").get()

        attributes = list()
        items["attributes"] = attributes

        items["about"] = response.xpath("//div[@class='productView-short-description']/p/text()").get()
        ...
        items["source"] = 'tumbltrak.com'

        images_list = response.xpath("//li[@class='productView-thumbnail']/a/div[@class='image']/@style").getall()
        for n in range(0,len(images_list)):
            images_list[n]=images_list[n].split("(")[-1]
            images_list[n]=images_list[n].split(")")[0]
