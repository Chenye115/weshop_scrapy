...
website = 'worpswede-kunst'
# 全流程解析脚本

class worpswede_kunst(scrapy.Spider):
    name = website
    ...
    def __init__(self, **kwargs):
        super(worpswede_kunst, self).__init__(**kwargs)
        self.counts = 0
    ...
    def parse(self, response):
        url_list = response.xpath("//nav[@class='navbar-categories-left']//a/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//div[@class='product-container']/form//div[@class='title']/a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
        ...
        next_page_url = response.xpath("//a[@title=' nächste Seite ']/@href").get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        current_price = response.xpath("//div[@class='current-price-container']/span/@content").get()
        ...
        items["brand"] = response.xpath("//div[@id='navbar-brand']/a/@title").get()
        items["name"] = response.xpath('//span[@itemprop="name"]/text()').get()
        # attributes = list()
        # items["attributes"] = attributes
        # items["about"] = response.xpath("").get()
        des=response.xpath('//div[@itemprop="description"]/font/text()').get()
        items["description"] =des

        items["source"] = 'worpswede-kunst.com'
        images_list = response.xpath("//div[@id='product_image_swiper']//a/@href").getall()
        for n in range(len(images_list)):
            images_list[n]='https://www.worpswede-kunst.com/'+images_list[n]
        items["images"] = images_list

        Breadcrumb_list = response.xpath("//div[@id='breadcrumb_navi']//span[@itemprop='name']/text()").getall()
        ...
        sku_list = list()
        have_sku = response.xpath("//form[@class='form-horizontal js-product-form product-info']//label/text()").getall()
        have_sku.pop(0)
        if len(have_sku)>0:
            print('hassku',len(have_sku))
            sku_option_list = response.xpath("//form[@class='form-horizontal js-product-form product-info']//select")
            sku_dict = {}
            for i,sku_option in enumerate(sku_option_list):
                sku_title = self.filter_text(self.filter_text(have_sku[i].strip()))
                print(sku_title)
