...
website = 'zondervanacademic'
# 全流程解析脚本

class zondervanacademic(scrapy.Spider):
    name = website
    ...
    def __init__(self, **kwargs):
        super(zondervanacademic, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        url_list = [
            'https://zondervanacademic.com/products',
    ...
    def parse(self, response):
        url_label_list = response.xpath("//ul[@class='t-books__categories-list -primary']//a")

        for url_label in url_label_list:
            cata=url_label.xpath("./span/text()").get()
            print('cata')
            print(cata)
    ...
    def parse_list(self, response):
        ...
        url_list = response.xpath("//div[@class='t-books__grid']/div//a/@href").getall()
        ...
        next_page_url = response.xpath("//a[@class='c-pagination__next']/@href").get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath('//span[@data-element="product.compareAt"]/text()').get()
        current_price = response.xpath('//span[@data-element="product.price"]/text()').get()
        ...
        items["brand"] = ''
        items["name"] = response.xpath("//h1[@class='t-product-detail__quick-title']/text()").get()
        attributes = list()
        attr_label=response.xpath("//ul[@class='t-product-detail__more-list']/li")
        attr_label.pop(0)
        for label in attr_label:
            attr_key=label.xpath("./span[1]/text()").get()
            attr_value=label.xpath("./span[2]/text()").get()
            attributes.append(self.filter_text(self.filter_html_label(attr_key))+self.filter_text(self.filter_html_label(attr_value)))
        items["attributes"] = attributes
        # items["about"] = response.xpath("").get()
        des= response.xpath("//div[@id='tdOverview']//text()").getall()
        des=' '.join(des)
        items["description"] =self.filter_text(self.filter_html_label(des))

        items["source"] ='zondervanacademic.com'
        images_list = response.xpath("//div[@class='t-product-detail__left']//img/@src").getall()
