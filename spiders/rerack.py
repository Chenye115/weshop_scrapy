...
class RerackSpider(scrapy.Spider):
    name = website
    allowed_domains = ['rerack.com']
    start_urls = ['https://www.rerack.com/']
    ...
    def __init__(self, **kwargs):
        super(RerackSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def parse(self, response):
        url_list = response.xpath("//ul[@data-navigation-menu='categories']//a/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//div[@class='product-grid grid-container']//a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
        ...
        next_page_url = response.xpath("//nav[@class='pagination']/a[@class='pagination-item pagination-next']/@href").get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        #/html/body/div[3]/main/div[3]/div[2]/div[2]/div/div/span
        #//div[@class='price-withtax']/div[class='price-value-wrapper']/span//text()
        current_price = response.xpath("//div[@class='price-value-wrapper']/span/text()").getall()
        ...
        #current_price=current_price.split("$")[-1]
        original_price = response.xpath("//div[@class='price-ns-wrapper']/span//text()").getall()
        ...
        brand= response.xpath("//a[@class='product-brand']/text()").get()
        items["brand"]=self.filter_text(self.filter_html_label(brand))
        ...
        des= response.xpath("//section[@id='description']//text()").getall()
        des=' '.join(des)
        des=self.filter_text(self.filter_html_label(des))
        items["description"] =des

        items["source"] = 'rerack.com'
        images_list = response.xpath("//div[@class='product-image-slides']//a/@href").getall()
        images_list=list(set(images_list))
        items["images"] = images_list

        Breadcrumb_list = response.xpath("//div[@class='breadcrumbs']//text()").getall()
        br=''.join(Breadcrumb_list)
        br=self.filter_text(self.filter_html_label(br))
        items["cat"] = br.split("/")[-1]
        items["detail_cat"] = br

        sku_list = list()
        have_sku = response.xpath("//div[@class='product-options']/div")
        print(have_sku)
        if len(have_sku)>0:
            print('hassku')
            sku_dict = {}
            for i in range(len(have_sku)):
                sku_title = have_sku[i].xpath(".//div[@class='form-field-title']/text()").get()
                sku_title=self.filter_text(self.filter_html_label(sku_title))
                # sku_title=''.join(sku_title)
                sku_opts = response.xpath(".//div[@class='form-field-control']/label/span/text()").getall()
