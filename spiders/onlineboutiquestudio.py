...
website = 'onlineboutiquestudio'
# 全流程解析脚本

class OnlineboutiquestudioSpider(scrapy.Spider):
    name = website
    allowed_domains = ['onlineboutiquestudio.com']
    start_urls = ['https://onlineboutiquestudio.com/']
    ...
    def __init__(self, **kwargs):
        super(OnlineboutiquestudioSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        url_list = [
            'https://onlineboutiquestudio.com/',
        ]
        for url in url_list:
            print(url)
        ...
        #url = "http://onlineboutiquestudio.com/"
        #yield scrapy.Request(
        #    url=url,
        #)

    def parse(self, response):
        url_list = response.xpath("//div[@class='mainmenu']//a/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath("//form[@id='form_singleProduct']/input[@name='_salePrice_nc']/@value").get()
        #original_price=original_price.split("$")[-1]
        current_price = response.xpath("//form[@id='form_singleProduct']/input[@name='_price_nc']/@value").get()
        ...
        #wrap-content
        des = response.xpath("//div[@class='wrap-content']/div[1]//text()|//div[@class='wrap-content']/div[1]//text()").getall()
        des=' '.join(des)
        des=self.filter_text(self.filter_html_label(des))
        items["description"]=des


        items["source"] = 'onlineboutiquestudio.com'
        images_list = response.xpath("//div[@class='item_slider']/div/div/@data-zoom-image").getall()
        if images_list:
            items["images"] = images_list
        else:
            images_list=response.xpath("//div[@class='single_showroom']/img/@data-zoom-image").getall()
            items["images"] = images_list
