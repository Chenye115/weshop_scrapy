...
class WentenySpider(scrapy.Spider):
    name = website
    allowed_domains = ['wenteny.com']
    start_urls = ['https://www.wenteny.com/']
    ...
    def __init__(self, **kwargs):
        super(WentenySpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        ...
        #url = "http://wenteny.com/"
        #yield scrapy.Request(
        #    url=url,
        #)

    def parse(self, response):
        url_list = response.xpath("//div[@id='pt_custommenu']/div/div/div/a/@href").getall()
        url_list.pop(0)
        url_list.pop(-1)
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//div[@class='custom-products']/div/div/div/div/a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath("//ul[@class='list-unstyled price-product']/li/span/text()").get()
        ...
        #tab-description
        attributes = []
        attr_list = response.xpath("//div[@id='tab-description']/div[@class='list_elem']/div")
        attr_list.pop(0)
        for attr in attr_list:
            attr_key = attr.xpath("./div/div/b/text()").get()
            attr_value = attr.xpath("./div/div/span/text()").get()
            attributes.append(self.filter_html_label(attr_key)+self.filter_html_label(attr_value))
        items["attributes"] = attributes

        ab=response.xpath("//p[@class='short-des']/text()").get()
        about=self.filter_text(self.filter_html_label(ab))
        items["about"] = about

        des=response.xpath("//ul[@class='radiobutton contentFull']/li/text()").getall()
        des=''.join(des)
        items["description"] = des

        items["source"] = 'wenteny.com'

        images_list = response.xpath("//div[@id='gallery_01']/a/img/@src").getall()
        items["images"] = images_list

        Breadcrumb_list = response.xpath("//ul[@class='breadcrumb']/li/a/text()").getall()
        strr='/'
