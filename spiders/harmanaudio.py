...
class harmanaudio(scrapy.Spider):
    name = website
    ...
    def __init__(self, **kwargs):
        super(harmanaudio, self).__init__(**kwargs)
        self.counts = 0
    ...
    def parse(self, response):
        url_list = response.xpath("//div[@class='navigation-table']//a/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//div[@id='search-result-items']/div//a[@class='thumb-link']/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
