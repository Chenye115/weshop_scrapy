...
class dfclau(scrapy.Spider):
    name = website
    #allowed_domains = ['dfclau.com/']
    start_urls = ['https://www.dfclau.com/']
    ...
    def __init__(self, **kwargs):
        super(dfclau, self).__init__(**kwargs)
        self.counts = 0
    ...
    def parse(self, response):
        url_list=[]
        url_list_temp = response.xpath("//nav[@id='menu']/div[2]//a/@href").getall()
        print(url_list_temp)
    ...
    def parse_list(self, response):
        print('解析页表')
        """列表页"""
        url_list = response.xpath("//div[@class='row catebg']/div/div/div/a[1]/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath("//h2[@class='pro-price']/text()").get()
        ...
        items["brand"] = ''
        items["name"] = response.xpath('//meta[@itemprop="name"]/@content').get()
        # attributes = list()
        ...
        items["source"] = 'dfclau.com'
        images_list = response.xpath('//li[@id="additional"]//a/@data-zoom-image').getall()
        ...
        sku_list = list()
        have_sku = response.xpath("//div[@class='form-group required ']//label/text()").getall()
        if len(have_sku)>0:
            sku_option_list = response.xpath("//div[@class='form-group required ']//select")
            sku_dict = {}
