...
website = 'minoribeauty'
# 全流程解析脚本

class minoribeauty(scrapy.Spider):
    name = website
    ...
    def __init__(self, **kwargs):
        super(minoribeauty, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        url_list = [
            'https://www.minoribeauty.com/shop',
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//div[@class='shop-section_product__3WL-N']//a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
