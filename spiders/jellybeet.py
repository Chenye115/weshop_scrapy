...
class JellybeetSpider(scrapy.Spider):
    name = website
    allowed_domains = ['jellybeet.com']
    start_urls = ['https://jellybeet.com/']
    ...
    def __init__(self, **kwargs):
        super(JellybeetSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        ...
        #url = "http://jellybeet.com/"
        #yield scrapy.Request(
        #    url=url,
        #)

    def parse(self, response):
        url_list = response.xpath("//ul[@id='navPages-main']/li[4]/div/ul/li/a/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//section[@class='products_view']/ul/li[@class='product']/article/div/figure/a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
        ...
        next_page_url=''
        next_page_label = response.xpath("//ul[@class='pagination-list']/li[last()]/a/i")
        if(len(next_page_label)==1):
            next_page_url=next_page_label.xpath("../@href").get()
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath("//div[@itemprop='offers'][1]/span[3]/text()").get().split("$")[-1]
        ...
        attributes = list()
        attributes_path_dt=response.xpath("//dl[@class='productView-info']/dt")
        attributes_path_dd=response.xpath("//dl[@class='productView-info']/dd")
        attributes_num=len(attributes_path_dt)
        for n in range(attributes_num):
            attributes.append(attributes_path_dt[n].xpath("./text()").get()+self.filter_text(self.filter_html_label(attributes_path_dd[n].xpath("./text()").get())))
        ...
        items["source"] = 'jellybeet.com'

        images_list = response.xpath("//section[@class='productView-images productView-images--bottom']//a/@href").getall()
