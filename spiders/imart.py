...
class ImartSpider(scrapy.Spider):
    name = website
    allowed_domains = ['imart.net']
    start_urls = ['https://imart.net/']
    ...
    def __init__(self, **kwargs):
        super(ImartSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def parse(self, response):
        url_list = response.xpath("//div[@class='mainmenu_cont']//a/@href").getall()
        url_list.pop(0)
    ...
    def parse_list(self, response):
        """列表页"""
        print('1')
        url_list = response.xpath("//div[@class='products_cont js-list_product']//a/@href").getall()
        print(url_list)
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
        ...
        next_page_url = response.xpath("//div[@class='adappagercont']/a[@class='next page-numbers']/@href").get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath("//form[@id='form_singleProduct']/input[@name='price']/@value").get()
        if original_price:
            original_price=original_price.split("$")[-1]
        current_price = response.xpath("//form[@id='form_singleProduct']/input[@name='salePrice']/@value").get()
        ...
        items["brand"] = 'I Mart'
        #items["name"] = response.xpath("//div[@class='item_adap_slider']//h1/text()").get()#itemprop="name"
        ...
        des=response.xpath("//div[@itemprop='description']//text()").getall()
        des=''.join(des)
        des=self.filter_text(self.filter_html_label(des))
        items["description"] = des

        items["source"] = 'imart.net'
        images_list = response.xpath("//div/@data-zoom-image").getall()
        items["images"] = images_list

        Breadcrumb_list = response.xpath("//div[@class='pr-breadcrumbs']//text()").getall()
        ...
        sku_list = list()
        have_sku = response.xpath("//div[@data-select='Please select']//div[@class='name']/text()").getall()
        if len(have_sku)>0:
            print('hassku')
            sku_option_list = response.xpath("//div[@data-select='Please select']//div[@class='value_cont']")
            sku_dict = {}
            for i,sku_option in enumerate(sku_option_list):
                sku_title = have_sku[i].strip()
                sku_opts = sku_option.xpath(".//span/@data-title").getall()
