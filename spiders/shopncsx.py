...
class shopncsx(scrapy.Spider):
    ...
    def __init__(self, **kwargs):
        super(shopncsx, self).__init__(**kwargs)
        self.counts = 0
    ...
    def parse(self, response):
        url_list = response.xpath("//ul[@class='module-list cat-nav']/li/a/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//table[@class='product-list']//a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath('//span[@class="prod-detail-cost-value"]/text()').get()
        ...
        items["name"] = response.xpath("//div[@id='product-detail-div']/h1/text()").get()
        items["brand"] = ''
        # attributes = list()
        # items["attributes"] = attributes
        # items["about"] = response.xpath("").get()
        des=response.xpath("//div[@class='prod-detail-desc']//text()").getall()
        ...
        items["source"] = 'shopncsx.com'
        images_list = response.xpath("//td[@class='prod-detail-lt']//a/@href").getall()
        for n in range(len(images_list)):
            images_list[n]='http://www.shopncsx.com/'+images_list[n]
        images_list=list(set(images_list))
        items["images"] = images_list

        Breadcrumb_list = response.xpath("//div[@class='breadcrumb']//text()").getall()
        br_list=self.filter_text(self.filter_html_label(''.join(Breadcrumb_list))).replace(">","/")
