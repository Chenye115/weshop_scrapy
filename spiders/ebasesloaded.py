...
website = 'ebasesloaded'
# 全流程解析脚本

class EbasesloadedSpider(scrapy.Spider):
    name = website
    allowed_domains = ['ebasesloaded.com']
    start_urls = ['https://ebasesloaded.com/']
    ...
    def __init__(self, **kwargs):
        super(EbasesloadedSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        ...
        #url = "http://ebasesloaded.com/"
        #yield scrapy.Request(
        #    url=url,
        #)

    def parse(self, response):
        url_list = response.xpath("//nav[@class='navbar px-0']//a/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        #无翻页
        url_list = response.xpath("//ul[@class='productGrid']/li[@class='product']/article/figure/a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath("//div[@class='productView-price d-lg-none']//span[@class='price price--withoutTax']/text()").get()
        ...
        items["source"] = 'ebasesloaded.com'

        images_list = response.xpath("//div[@class='slick-track']/li/a/@href").getall()
        items["images"] = images_list

        Breadcrumb_list = response.xpath("//ul[@class='breadcrumbs container']/li/a/span/text()|//ul[@class='breadcrumbs container']/li/span/text()").getall()
