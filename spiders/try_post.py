...
class kuranda(scrapy.Spider):
    name = website
    # allowed_domains = ['kuranda']
    start_urls = 'https://kuranda.com/'
    ...
    def __init__(self, **kwargs):
        super(kuranda, self).__init__(**kwargs)
        self.counts = 0
        setattr(self, 'author', "流冰")
        self.headers={'cookie': 'SHOP_SESSION_TOKEN=lp7cfj91887gmsffa9ta4s1a0i; fornax_anonymousId=9cc7d3a1-2625-43e8-a405-ab96101ba0f1; _gcl_au=1.1.1444715952.1640094035; ajs_group_id=null; ajs_user_id=null; ajs_anonymous_id=%22112af8f8-313d-497d-809b-93be33c43ac9%22; STORE_VISITOR=1; _ga=GA1.2.2083665111.1640094046; _gid=GA1.2.1102605884.1640094046; _fbp=fb.1.1640094046265.1869218946; XSRF-TOKEN=c1851a54b7ee78b47d3833ca41243bf7427dbe319a5577b7fd2290fbb32bf3d8; _clck=9b0vzr|1|exh|0; lastVisitedCategory=43; _uetsid=935051c0626311ec8b29d9b8d0d9ae34; _uetvid=9350a430626311ec81361943017d63c8; _clsk=1qldmap|1640146457864|5|1|e.clarity.ms/collect; Shopper-Pref=40A62E6F69B8A52A9A7116AD92F8B44A1171CECE-1640751282437-x%7B%22cur%22%3A%22USD%22%7D; _gali=attribute_rectangle__274_306', 'origin': 'https://kuranda.com', 'pragma': 'no-cache', 'sec-ch-ua': '" Not A;Brand";v="99", "Chromium";v="96", "Google Chrome";v="96"', 'sec-ch-ua-mobile': '?0', 'sec-ch-ua-platform': '"Windows"', 'sec-fetch-dest': 'empty', 'sec-fetch-mode': 'cors', 'sec-fetch-site': 'same-origin', 'stencil-config': '{}', 'stencil-options': '{"render_with":"products/bulk-discount-rates"}', 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36', 'x-requested-with': 'stencil-utils', 'x-xsrf-token': 'c1851a54b7ee78b47d3833ca41243bf7427dbe319a5577b7fd2290fbb32bf3d8'}
    ...
    def parse(self, response):
        url_list = response.xpath("//a[@class='navPage-subMenu-action navPages-action']/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//h3[@class='card-title']/a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
    ...
    def parse_detail(self, response):
        ...
        if not items["current_price"] or items["current_price"] == '0.00':
            return
        ...
        Breadcrumb_list = response.xpath("//ul[@class='breadcrumbs']//li//span/text()").getall()
        ...
        if sku_options:
            ...
            if len(org_sku_list) > 0 and len(org_sku_list) <= 200:
                for sku in org_sku_list:
                    post_url = 'https://kuranda.com/remote/v1/product-attributes/{}'.format(product_id)
                    post_data = {
                        'action': 'add',
                    ...
                    #
                    sku_data = post_res_json.get("data")
                    try:
            ...
            elif len(org_sku_list) > 0 and len(org_sku_list) > 200:
                for sku in org_sku_list:
                    sku_item = SkuItem()
                    other = dict()
                    attributes = SkuAttributesItem()
