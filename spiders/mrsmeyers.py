...
class MrsmeyersSpider(scrapy.Spider):
    name = website
    allowed_domains = ['mrsmeyers.com']
    start_urls = ['http://mrsmeyers.com/']
    ...
    def __init__(self, **kwargs):
        super(MrsmeyersSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        url_list = [
            'https://www.mrsmeyers.com/product/'
        ]
        for url in url_list:
            print(url)
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//ul[@class='productGrid']//li/article/a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
    ...
    def parse_detail(self, response):
        ...
        items["brand"] = 'Mrs. Meyers Scent'
        items["name"] = response.xpath("//div[@class='productView-product']//h1/text()").get()
        attributes = list()
        items["attributes"] = attributes
        items["about"] = response.xpath("//section[@class='productView-details']/article/div[2]/ul//li/text()").get()
