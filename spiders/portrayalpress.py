...
website = 'portrayalpress'
# 全流程解析脚本

class PortrayalpressSpider(scrapy.Spider):
    name = website
    allowed_domains = ['portrayalpress.com']
    start_urls = ['https://www.portrayalpress.com/']
    ...
    def __init__(self, **kwargs):
        super(PortrayalpressSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        ...
        #url = "http://portrayalpress.com/"
        #yield scrapy.Request(
        #    url=url,
        #)

    def parse(self, response):
        url_list = response.xpath("//ul[@class='vnav vnav--horizontal vnav--level1']//a/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//form[@id='MainForm']//a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
    ...
    def parse_detail(self, response):
        ...
        attributes = response.xpath("//span[@itemprop='product_code_title']/text()|//span[@itemprop='product_code']/text()").getall()
        attributes=''.join(attributes)
        items["attributes"] = attributes


        des= response.xpath("//table[@class='colors_descriptionbox']//text()").getall()
        des=' '.join(des)
        des=self.filter_text(self.filter_html_label(des))
        items["description"]=des

        items["source"] ='petpopulars.com'
        ...
        Breadcrumb_list = response.xpath("//table[@id='v65-product-parent']/tr[1]/td/b/a/text()").getall()
