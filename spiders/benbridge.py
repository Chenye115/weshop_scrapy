...
class BenbridgeSpider(scrapy.Spider):
    name = website
    allowed_domains = ['benbridge.com']
    start_urls = ['http://benbridge.com/']
    ...
    def __init__(self, **kwargs):
        super(BenbridgeSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        ...
        #url = "http://benbridge.com/"
        #yield scrapy.Request(
        #    url=url,
        #)

    def parse(self, response):
        url_list = response.xpath("//nav[@id='navigation']//ul[@class='level-3']/li/a/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//div[@class='search-result-content']/ul/li/div/div[2]/a/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
