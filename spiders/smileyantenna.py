...
website = 'smileyantenna'
# 全流程解析脚本

class SmileyantennaSpider(scrapy.Spider):
    name = website
    allowed_domains = ['smileyantenna.com']
    start_urls = ['https://www.smileyantenna.com/']
    ...
    def __init__(self, **kwargs):
        super(SmileyantennaSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    def start_requests(self):
        ...
        #url = "http://smileyantenna.com/"
        #yield scrapy.Request(
        #    url=url,
        #)

    def parse(self, response):
        url_list = response.xpath("//div[@id='display_menu_1']//a/@href").getall()
    ...
    def parse_list(self, response):
        """列表页"""
        url_list = response.xpath("//div[@class='v-product-grid']/div/a[1]/@href").getall()
        url_list = [response.urljoin(url) for url in url_list]
        for url in url_list:
            print(url)
            yield scrapy.Request(
    ...
    def parse_detail(self, response):
        ...
        original_price = response.xpath("//font[@class='pricecolor colors_productprice']/div[@class='product_productprice']/b/span/text()").get()
        ...
        des = response.xpath("//span[@itemprop='description']/text()").get()
        des=self.filter_text(self.filter_html_label(des))
        items["description"]=des

        items["source"] = 'smileyantenna.com'
        ...
        sku_list = list()
        have_sku = response.xpath("//table[@id='options_table']/tr[2]/td[1]")
        print(have_sku)
        if len(have_sku)>0:
            print('hassku')
            sku_option_list = response.xpath("//table[@id='options_table']/tr[2]//select")
            sku_dict = {}
            for i,sku_option in enumerate(sku_option_list):
                title=have_sku[i].xpath(".//text()").getall()
                title=''.join(title)
                title=self.filter_text(self.filter_html_label(title))
