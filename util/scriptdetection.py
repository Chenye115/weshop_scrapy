        ...
        if mustparam_k == 'sku_list':
            ...
            if skulist:
                if 'sku_list' in items and items['sku_list']:
                    for sku in items['sku_list']:
                        if mustparam_v['original_price'] and mustparam_v['current_price']:
                            ori_price = sku.get('original_price')
                            cur_price = sku.get('current_price')
                            if ori_price and cur_price:
                                if float(ori_price) < float(cur_price):
                                    my_detection.append('【sku.price】现价大于原价, 价格有问题! 请注意T.T')
                            elif not ori_price or not cur_price:
                                my_detection.append('【sku.price】未抓全! 请注意T.T')
                        for check_field, state in mustparam_v.items():
                            if check_field == 'imgs':
                                pass
                            elif check_field == 'img':
                                if state and 'imgs' not in sku:
                                    my_detection.append('【sku_list.imgs】重要字段未抓取到！请检查T.T')

                            else:
                                if state and check_field not in sku:
                                    my_detection.append('【sku_list.' + str(check_field) + '】重要字段未抓取到！请检查T.T')
                            ...
                            if sku_validation:
                                ...
                                if sku_field == 'imgs':
                                    ...
                                    if mustparam_v[sku_field]:
                                        check_img_size(my_detection, sku, sku_field, whole_zong_img_num,
                                                       whole_small_img_num, whole_large_img_num, whole_res_img_num)
                                    else:
                                        if not check_img_size_False(my_detection, sku, sku_field,sku=True):
                                            break
                                else:
                                    if mustparam_v[sku_field]:
            ...
            if mustparam_v:
                ...
                if parameter or parameter == 0:
                    ...
                    if mustparam_k == 'name' or mustparam_k == 'brand':
